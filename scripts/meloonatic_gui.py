...
class Menu:  
    ...
    class Button:
        ...
        All = []  # Generate list of all buttons made

        def __init__(self, text, rect, bg, fg, bgr, bgc,  font=Font.Default, tag=("menu", None)):
            self.Text = text
            self.Left = rect[0]
            self.Top = rect[1]
            self.Width = rect[2]
            ...
            # CLICKED  BUTTON
            self.Click = pygame.Surface((self.Width, self.Height), pygame.HWSURFACE|pygame.SRCALPHA)
            self.Click.fill(bgc)
            self.Click.blit(RText, (self.Width / 2 - txt_rect[2] / 2, self.Height / 2 - txt_rect[3] / 2))
        ...
        def Render(self, to, pos = (0, 0)):
            if MouseOver((self.Left + pos[0], self.Top + pos[1], self.Width, self.Height)):
                ...
                self.Rolling = True
                if self.Clicked:
                    to.blit(self.Click, (self.Left + pos[0], self.Top + pos[1]))
